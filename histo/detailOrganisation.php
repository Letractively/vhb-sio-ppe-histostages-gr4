...
{
	ajouterErreur($tabErreurs, "La base de donnÈes stsig est inexistante ou non accessible");
	afficherErreurs($tabErreurs);
	exit();
}
...
{
	ajouterErreur($tabErreurs, "Absence de numÈro pour l'organisation ‡ dÈtailler");
	afficherErreurs($tabErreurs);
}
else
{
	...
	{
		ajouterErreur($tabErreurs,"Organisation demandÈe invalide ou inexistante");
		afficherErreurs($tabErreurs);
	}
	else
	{
		...
		<tr class="ligneTabNonQuad">
		<td class="intitule"> Libell√© cat√©gorie:</td>
		<td class="valeur">'.$libelleCat.'</td>
		</tr>
		<tr class="ligneTabNonQuad">
		<td class="intitule"> T√©l√©phone: </td>
